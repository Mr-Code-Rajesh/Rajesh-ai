...
export default function ChatModule({
  ...
  return (
    <div className="flex flex-col w-full flex-1 glass-card rounded-[2.5rem] bg-bg-dark/40 border border-white/10 overflow-hidden relative shadow-2xl transition-all duration-300">
      ...
      {messages.length > 0 && (
         <div className="px-8 py-4 border-b border-white/5 bg-white/2 flex items-center justify-between">
           <div className="flex items-center gap-3">
             ...
             <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Active Neural Session</span>
           </div>
           <span className="text-[10px] font-mono text-zinc-700 truncate max-w-[150px]">{infoEmail}</span>
         </div>
      )}
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-6 md:p-10 space-y-4 scrollbar-hide"
      >
        {messages.length === 0 && (
          <motion.div 
            initial={{ opacity: 0 }}
      ...
      {/* Input Container */}
      <div className="px-6 md:px-10 py-6 md:py-8 bg-bg-dark/60 backdrop-blur-md border-t border-white/5">
        <ChatInput onSend={onSendMessage} isLoading={isTyping} />
        <div className="mt-4 flex items-center justify-center gap-4 text-[9px] uppercase tracking-[0.2em] text-zinc-600 font-bold">
